...
DEFAULT_K = 5
SIMILARITY_THRESHOLD = 70.0  # Minimum similarity percentage (100 - Euclidean Distance)

# ==================== PINECONE CONFIGURATION ====================
PINECONE_API_KEY = "pcsk_3DmuwS_5NT3SrpWJW9cweaEduTnvociJPwvVQFBYaJsbdc88joQoKk3JvSmRVJ6jSeiUfZ"
PINECONE_INDEX_NAME = "movie-similarity-v4-euclid-final"
PINECONE_NAMESPACE = "movie-growth-vectors-v4-euclid"
VECTOR_DIMENSION = 60
    ...
    try:
        ...
        if PINECONE_INDEX_NAME in index_names:
            ...
            pc.create_index(
                ...
                metric="euclidean",  # Changed to Euclidean for magnitude-aware similarity
                spec=ServerlessSpec(
                    cloud="aws",
                    region="us-east-1"
                )
...
def build_movie_database():
    ...
    try:
        ...
        # CRITICAL FIX: Use RAW growth values (no normalization)
        # L2 normalization was causing movies with similar active days to appear similar
        # even when their growth patterns were completely different
        print("🔄 Preparing embeddings (raw growth values - no normalization)...")
...
def initialize_system():
    ...
    try:
        ...
        df_full['dbr'] = pd.to_numeric(df_full['dbr'], errors='coerce').fillna(0).astype(int)
        df_full['Growth (%)'] = pd.to_numeric(df_full['Growth (%)'], errors='coerce').fillna(0)
...
def search_similar_movies(movie_title: str, k: int = DEFAULT_K) -> Dict[str, Any]:
    ...
    try:
        ...
        query_growth = embeddings[query_idx]  # Already raw growth values
        ...
        for match in search_results.matches:
            ...
            # match.score is the SQUARED distance in Pinecone
            distance = math.sqrt(float(match.score))
            
            # Convert distance to similarity percentage
            # Assuming distance is roughly 0-100 scale for growth % differences
            # 100 - distance gives a reasonable similarity score
            similarity_pct = max(0.0, 100.0 - distance)
            ...
            sim_growth = embeddings[sim_idx]  # Already raw growth values
            ...
            results.append({
                'title': sim_title,
                'confidence': round(similarity_pct / 100.0, 3),  # Convert to 0-1 for compatibility
        ...
        if not results:
            return {
                ...
                "message": f"No similar movies found (threshold: {SIMILARITY_THRESHOLD}%)"
            }
...
"""
        ...
        for i, movie in enumerate(similar_movies[:5], 1):  # Top 5 only for analysis
            ...
            context += f"\n   - Movie full data: {str(movie.get('movie_data', 'N/A'))}"
...
def format_results(search_results: Dict[str, Any]) -> str:
    ...
    for i, movie in enumerate(search_results['similar_movies'], 1):
        ...
        # output += f"   Movie Data: {str(movie.get('movie_data', 'N/A'))}%\n"
...
# ==================== GRAPH GENERATION ====================
def get_growth_curve(title):
    """Calculate growth curve for a movie exactly as the bot does (active days only)"""
    movie_data = df_full[df_full['title'].str.lower() == title.lower()].copy()
    if movie_data.empty:
        return None, None
        
    # Ensure numeric
    movie_data['daily_revenue'] = pd.to_numeric(movie_data['daily_revenue'], errors='coerce').fillna(0)
    
    # Use 'dbr' column (which represents the day)
    group = movie_data.groupby('dbr').agg({
        'daily_revenue': 'sum'
    }).reset_index().sort_values('dbr')
    
    # Filter active days (same logic as initialize_system)
    active = group[group['daily_revenue'] > 100]
    if len(active) < 2:  # Need at least 2 days for growth
        return None, None
    ...
    # Return days (1 to N) and growth
    days = np.arange(1, len(growth) + 1)
    return days, growth

def generate_graph(query_movie_title: str, similar_movies: list):
    """Generate and save similarity graph using ACTUAL growth curves"""
    try:
        plt.figure(figsize=(12, 6))
        
        # Plot Query Movie
        days, growth = get_growth_curve(query_movie_title)
        if days is not None:
            plt.plot(days, growth, label=f"Query: {query_movie_title}", linewidth=3, color='blue', marker='o')
        ...
        for i, movie in enumerate(similar_movies):
            ...
            days, growth = get_growth_curve(title)
            
            if days is not None:
                color = colors[i % len(colors)]
                # Add confidence to label
                label = f"{i+1}. {title} ({int(confidence*100)}%)"
                plt.plot(days, growth, label=label, linestyle='--', alpha=0.7, color=color, marker='x')
        
        plt.title(f"Growth Similarity: {query_movie_title} (Euclidean Distance)")
        plt.xlabel("Active Day Number")
        ...
        # Save graph
        # Sanitize filename
        safe_title = re.sub(r'[^a-zA-Z0-9]', '_', query_movie_title)
        filename = f"{safe_title}_similarity.png"
        save_path = os.path.join("similarity_graphs_v2", filename)
...
def ask(user_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        graph_path = generate_graph(search_results['query_movie'], search_results['similar_movies'])
...
def main():
    """Main entry point"""
    print("\n" + "="*70)
    print("🎬 MOVIE SIMILARITY SEARCH - V4 (With Graphs) - FIXED")
    print("="*70)
