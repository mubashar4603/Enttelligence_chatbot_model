#!/usr/bin/env python3
"""
Thread-based parallel pipeline for processing Movie data and creating embeddings
"""
...
class ThreadBasedParallelSystem:
    """Thread-based parallel pipeline for creating embeddings from Movie models"""
    
    def __init__(self, csv_path=None):
        """Initialize the thread-based parallel pipeline
...
""".strip()
    ...
    def process_chunk(self, movies, chunk_num):
        ...
        with self.lock:
            current_start = self.processed_count
            self.processed_count += len(texts)
        
        self.upload_vectors(embeddings, metadata_list, current_start)
    ...
    def get_movie_queryset(self):
        ...
        if self.csv_path and self.csv_unique_keys:
            ...
            # We'll filter by creating a queryset that matches the unique combinations
            from django.db.models import Q
            from django.db.models.functions import Concat
            from django.db.models import CharField, Value
            
            # Build Q objects for each unique key
            # This is more efficient than iterating through all keys
            # Instead, we'll use a combination of filters
            ...
            # We'll do this by creating a list of IDs that match
            matching_ids = []
    ...
    def run_thread_parallel_pipeline(self):
        """Run the thread-based parallel pipeline"""
        logger.info("🚀 Starting Thread-Based Parallel Pipeline")
        if self.csv_path:
        ...
        logger.info(f"🧵 Using {self.config['NUM_THREADS']} threads")
        logger.info("")
        
        # Process chunks sequentially (threading can be added later if needed)
        ...
        logger.info("=" * 80)
        logger.info("✅ THREAD-BASED PARALLEL PIPELINE COMPLETE!")
        logger.info("=" * 80)
